...
public class InstantSwapClient {
    ...
    private static final KeyMapping.Category SWAP_CATEGORY =
            KeyMapping.Category.register(
                    Identifier.fromNamespaceAndPath("susinstantswap", "general")
            );
    ...
    private static void performSurvivalSwap(Minecraft mc) {
        ...
        short slotIndex = (short) hovered.index;
        if (slotIndex < 9 || slotIndex > 44) {
            LOGGER.warn("[SusInstantSwap] 生存交换失败: slotIndex={}", slotIndex);
            return;
        }

        byte hotbar = (byte) mc.player.getInventory().getSelectedSlot();
    ...
    private static void performCreativeSwap(Minecraft mc) {
        ...
        if (!isPlayerInventorySlot(hovered, mc)) {
            ...
            mc.player.getInventory().setItem(selected, item.copy());
            didSwap = true;
            ...
            mc.player.getInventory().setItem(selected, targetItem.copy());

            if (!heldItem.isEmpty()) {
                mc.gameMode.handleCreativeModeItemAdd(heldItem, realIndex);
                int invIndex = (realIndex >= 36) ? realIndex - 36 : realIndex;
                mc.player.getInventory().setItem(invIndex, heldItem.copy());
            }
            didSwap = true;
