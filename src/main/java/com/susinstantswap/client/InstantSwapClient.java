...
public class InstantSwapClient {
    ...
    public static void init(SwapConfigAdapter cfg) {
        ...
        TickEvent.ClientTickEvent.Post.BUS.addListener(InstantSwapClient::onClientTickPost);
    }
    ...
    public static void onScreenKeyPressedPre(ScreenEvent.KeyPressed.Pre event) {
        ...
        for (InputConstants.Key target : SwapKeyState.getTargetKeys()) {
            if (target.getType() == InputConstants.Type.KEYSYM
                ...
                // Forge 61.x EventBus 7: Pre event no longer has setCanceled.
                // Block handled by ScreenKeyMixin instead.
                return;
            }
        }
    }
    ...
    private static void onClientTickPost(TickEvent.ClientTickEvent.Post event) {
        ...
        // GUI-swap (physical GLFW poll). ⚠ On 1.21.1 the onKeyInput
        // (InputEvent.Key) event does NOT fire while a container screen is open,
        // so the key-event path below is inert for GUI-swap in-screen — this
        // poll is the ONLY working path when the GUI-swap key differs from the
        // inventory key. (Differs from 1.20.1, where InputEvent.Key still fires
        // in-screen.) The poll's rising-edge guard (down && !wasDown) makes it
        // fire exactly once per press, so there is no double-swap here.
        // Skipped when GUI key == a target key (ScreenKeyMixin handles that).
