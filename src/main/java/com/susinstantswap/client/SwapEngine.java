...
public final class SwapEngine {
    ...
    private static boolean creativeSwap(Minecraft mc, CreativeModeInventoryScreen cs,
        ...
        if (hs.container == getCreativeContainer()) {
            ...
            ItemStack item = hs.getItem().copy();
            item.setCount(1);
            if (!held.isEmpty()) {
                int f = freeSlot(mc);
        ...
        if (cs.isInventoryOpen() && (csi == 45 || (csi >= 5 && csi <= 8))) {
            ...
            if (csi <= 8 && !handStack.isEmpty()) {
                ...
                EquipmentSlot actual = Mob.getEquipmentSlotForItem(handStack);
                if (actual.getType() != EquipmentSlot.Type.ARMOR || actual != expected) {
                    SwapToast.warn("toast.susinstantswap.item_not_placeable");
                    return false;
                }
            }
    ...
    // ── Creative inventory internals ──
    // CreativeModeInventoryScreen.CONTAINER is AT'd to public at runtime.
    // Forge 1.20.1: dev uses Mojang name "CONTAINER", runtime uses SRG "f_98505_".
    ...
    private static Object getCreativeContainer() {
        if (!creativeContainerCached) {
            ...
            try {
                Field f;
                try {
                    f = CreativeModeInventoryScreen.class.getDeclaredField("CONTAINER");
                } catch (NoSuchFieldException e) {
                    f = CreativeModeInventoryScreen.class.getDeclaredField("f_98505_");
                }
